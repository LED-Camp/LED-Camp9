...
LineSensor::LineSensor(Robot *robot, std::string sensorNameLeft,
...
LineSensor *LineSensor::getInstance(Robot *robot, std::string sensorNameLeft,
                                    std::string sensorNameCenter,
                                    std::string sensorNameRight, int timeStep) {
    if(_instance == 0) {
        _instance = new LineSensor(robot, sensorNameLeft, sensorNameCenter,
                                   sensorNameRight, timeStep);
    }
    return _instance;
}
